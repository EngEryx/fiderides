...
trait RoleAttribute
{
    ...
    public function getActionButtonsAttribute()
    {
        ...
        return '<div class="btn-group btn-group-xs" role="group" aria-label="User Actions">
			  '.$this->edit_button.'
			  '.$this->delete_button.'
			</div>';
    }
    ...
    public function getPermissionsListAttribute()
    {
        ...
        if ($this->id == 1){
            $result .= __('labels.general.all');
        } else {
            if ($this->permissions->count())
                foreach ($this->permissions as $permission){
                    $result .= ucwords($permission->name).'<br />';
                }
            else{
                $result .= '<span class="label label-danger">'.__('labels.general.none').'</span>';
            }
        }
