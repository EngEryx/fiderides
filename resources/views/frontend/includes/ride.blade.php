<ul class="list-group list-group-flush mt-1">
    ...
    @foreach($ride->destinations()->orderBy('order')->get() as $item)
        <li class="list-group-item py-2">Destination {{ ($ride->kind == 1) ? $loop->iteration : '' }} <span class="pull-right badge badge-success">{{ $item->destination }} - Kshs {{ number_format($item->amount, 2) }}</span></li>
    @endforeach
